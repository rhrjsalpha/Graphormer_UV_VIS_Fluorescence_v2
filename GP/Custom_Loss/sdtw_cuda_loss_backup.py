...
class _SoftDTWCUDA(Function):
    ...
    def forward(ctx, X, Y, D, gamma, bandwidth):
        ...
        # ✅ `D`를 detach()하여 numba.cuda 오류 방지
        D_detached = D.detach()
        ...
        compute_softdtw_cuda[B, threads_per_block](cuda.as_cuda_array(D_detached),
...
class SoftDTW(torch.nn.Module):
    ...
    def _get_func_dtw(self, x, y):
        ...
        """
        #print("sdtw x.shape",x.shape)
        #print("sdtw x.shape",y.shape)
        bx, lx, dx = x.shape # batchsize, seq len, demension
        by, ly, dy = y.shape
